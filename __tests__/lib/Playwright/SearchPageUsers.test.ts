...
test('Search for user using username handle', {tag: '@search'}, async ({ page }) => {
    ...
    // Type userHandle
    await page.getByRole('main').getByRole('search', { name: 'Search' }).fill(userHandle);
...
test('Search for user using first and last name', {tag: '@search'}, async ({ page }) => {
    ...
    // Type fullname
    await page.getByRole('main').getByRole('search', { name: 'Search' }).fill(fullname);
    ...
    // Assertion
    // Verify currect profile searched matches based on url
    await expect(currenUrl).toContain(`https://bsky.app/profile/${handlename.toLowerCase()}`);
